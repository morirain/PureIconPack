...
object SendIconRequest {
    ...
    suspend fun sendIconVotesRequest(context: Context, apps: ArrayList<RequestApp>, callback: ((result: AppVotes) -> Unit)? = null) {
        return withContext(IO) {
            val baseUrl = context.getString(R.string.request_manager_base_url)
            var succeeded = false
            val jsonContent = buildVotesRequestJson(apps)
            var result: AppVotes? = null
            try {
                getService(baseUrl).sendVotesRequest(jsonContent).let {
                    result = it.body()
                    succeeded = result?.status == "succeed"
                }
            } catch (e: HttpException) {
                e.message()
            } catch (e: Exception) {
    ...
    private fun buildVotesRequestJson(apps: ArrayList<RequestApp>): String {
        val correctList = ArrayList<Pair<String, RequestApp>>()
        for (app in apps) {
            val iconName = app.name.safeDrawableName()
            correctList.add(Pair(iconName, app))
        }
        val jsonSb = StringBuilder("{\"components\":[")
        var isFirst = true
        for ((_, app) in correctList) {
            if (!isFirst) jsonSb.append(",")
            jsonSb.append("{")
                .append("\"componentInfo\":\"${app.component}\"")
                .append("}")
